...
const DEFAULT_ORDER = [
  'weather','lunch','passHistory','checkmate','fortune','cowboyCode',
...
function CheckMateCard({ checkouts }) {
  ...
  function countdown(dueAt) {
    ...
    if (days === 0) return { label: `${hours}h left`, color: '#d97706', bg: '#fef3c7' }
    if (days === 1) return { label: '1 day left', color: '#d97706', bg: '#fef3c7' }
    if (days <= 3) return { label: `${days} days left`, color: '#d97706', bg: '#fef3c7' }
    return { label: `${days} days left`, color: RHS_GREEN, bg: '#e8f5ee' }
  }
  ...
  return (
    <Card>
      <CardHeader label="CheckMate Equipment" draggable />
      <CardBody compact>
        {checkouts.map((co, i) => {
          const cd = countdown(co.due_at)
          return (
            <div key={co.id} style={{
              display: 'flex', alignItems: 'center', gap: 10,
              padding: '7px 0',
              borderBottom: i < checkouts.length - 1 ? '0.5px solid #f0eeea' : 'none',
            }}>
              <div style={{
                width: 36, height: 36, borderRadius: 8,
                background: cd.bg, flexShrink: 0,
                display: 'flex', alignItems: 'center', justifyContent: 'center',
              }}>
                <i className="ti ti-device-laptop" style={{ fontSize: 18, color: cd.color }} />
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 13, fontWeight: 600, color: '#1a1a18',
                  whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                  {co.cm_equipment?.name || 'Equipment'}
                </div>
                <div style={{ fontSize: 10, color: '#aaa', marginTop: 1 }}>
                  Out {fmtDate(co.checked_out_at)} · Due {fmtDate(co.due_at)}
                </div>
              </div>
              <span style={{
                fontSize: 10, fontWeight: 600, color: cd.color,
                background: cd.bg, padding: '3px 8px', borderRadius: 8,
...
function WireContent() {
  ...
  const [cmCheckouts,  setCmCheckouts]  = useState([])
  ...
  // ── CheckMate equipment checkouts ────────────────────────────────────────────
  useEffect(() => {
    if (!student?.id) return
    async function loadCmCheckouts() {
      try {
        const { data: cmStudent } = await supabase
          .from('cm_students')
          .select('id')
          .eq('passable_id', student.id)
          .maybeSingle()
        if (!cmStudent) { setCmCheckouts([]); return }
        const { data: checkouts } = await supabase
          .from('cm_checkouts')
          .select('id, checked_out_at, due_at, cm_equipment(name, category)')
          .eq('student_id', cmStudent.id)
          .is('checked_in_at', null)
          .order('due_at', { ascending: true })
        setCmCheckouts(checkouts || [])
      } catch { setCmCheckouts([]) }
    }
    loadCmCheckouts()
    const id = setInterval(loadCmCheckouts, 30000)
    return () => clearInterval(id)
  }, [student?.id])
  ...
  const slotData = {
    ...
    checkmate:   cmCheckouts.length > 0 ? cmCheckouts : null,
  }
  ...
  function renderSlot(id) {
    ...
    switch (id) {
      ...
      case 'checkmate':    return <CheckMateCard key={id} checkouts={d} />
      default:            return null
    }
  }
