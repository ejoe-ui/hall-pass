/*
  ...
  URL:     https://hall-pass-lime.vercel.app/wire?room=27
...
function normalizeUid(raw) {
  // Preserves full UID — supports both 6-char Aeries student IDs and 8-char NFC hex UIDs.
  // Uppercase to match nfc_uid values stored in DB (e.g. '00006DC8').
...
const CO_REASONS = ['Restroom','Library','Lockers','Office','Counselor','Career Counselor','Errand','Class Assignment','IT / Tech Support']
...
function PassHistoryCard({
  ...
  // Destination teacher (for Errand / Class Assignment) — list comes from WireContent prop
  ...
  function resetCo() {
    ...
    setCoReason(''); setCoMsg(''); setCoCountdown(5)
  ...
  // Handle reason selection — route to special stages for Library / Errand / Class Assignment
  const WIRE_DEST_REASONS = ['Errand', 'Class Assignment']
  const WIRE_SHARED_REASONS = ['Office', 'Counselor', 'Career Counselor', 'IT / Tech Support']
  function selectReason(r) {
    setCoReason(r)
    if (r === 'Library') {
      setCoStage('libraryWarn')
    } else if (WIRE_DEST_REASONS.includes(r) || WIRE_SHARED_REASONS.includes(r)) {
      setCoDestTeacher(null); setCoDestTeacherSearch(''); setCoDestNote('')
      setCoStage('destInfo')
    }
    // Restroom / Lockers stay on 'found' stage — Check Out button becomes active
  }
  ...
  async function doCheckout() {
    ...
    try {
      ...
      // Build destination_note:
      // - For Errand/Class Assignment: "Teacher: Name · user note"
      // - For shared destinations (Office, Counselor, etc.): reason label + user note
      let combinedNote = null
      if (WIRE_SHARED_REASONS.includes(coReason)) {
        const parts = [coReason, coDestNote.trim()].filter(Boolean)
        combinedNote = parts.join(' — ') || null
      } else if (coDestTeacher) {
        const teacherPrefix = `Teacher: ${coDestTeacher.full_name}`
        const userNote = coDestNote.trim()
        combinedNote = [teacherPrefix, userNote].filter(Boolean).join(' · ') || null
      }
  ...
  const InlineCheckout = () => {
      ...
      <div style={{ borderTop: '0.5px solid #e8f0ec', background: '#f5f9f6', padding: '12px 13px' }}>
        ...
        <div style={{ display: 'flex', gap: 7 }}>
          ...
          <button onClick={doCheckout}
            disabled={!coReason || WIRE_DEST_REASONS.includes(coReason) || WIRE_SHARED_REASONS.includes(coReason) || coReason === 'Library'}
            style={{ flex: 2, fontSize: 13, fontWeight: 600, padding: '8px', borderRadius: 7, border: 'none',
              background: (coReason && !WIRE_DEST_REASONS.includes(coReason) && !WIRE_SHARED_REASONS.includes(coReason) && coReason !== 'Library') ? RHS_GREEN : '#e0ddd8',
    ...
    // ── Destination info (Errand / Class Assignment / Office / Counselor / Career Counselor / IT) ──
    if (coStage === 'destInfo') {
      const isShared = WIRE_SHARED_REASONS.includes(coReason)
      // Class Assignment requires a teacher; Errand optional; shared destinations — no teacher picker
      const canCheckOut = isShared ? true : (coReason !== 'Class Assignment' || !!coDestTeacher)

      return (
        <div style={{ borderTop: '0.5px solid #e8f0ec', background: '#f5f9f6', padding: '12px 13px' }}>
          {StudentHeader({ sub: isShared ? coReason : coReason + ' — who are you heading to?' })}

          {/* Teacher dropdown — only for Errand / Class Assignment */}
          {!isShared && (
            <>
              <p style={{ fontSize: 10, fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', color: '#888', marginBottom: 5 }}>
                Destination Teacher {coReason === 'Class Assignment' ? '' : <span style={{ fontWeight: 400, textTransform: 'none', letterSpacing: 0 }}>(optional)</span>}
              </p>
              <select
              ...
              >
                <option value="">{coReason === 'Class Assignment' ? '— Select a teacher (required) —' : '— Select a teacher (optional) —'}</option>
          ...
          <input value={coDestNote} onChange={e => setCoDestNote(e.target.value)}
            placeholder={
              coReason === 'Class Assignment' ? 'e.g. picking up assignment, borrowing materials…' :
              coReason === 'Errand' ? 'e.g. returning book, delivering message…' :
              'e.g. what you need help with…'
            }
            autoFocus={isShared}
          ...
          <div style={{ display: 'flex', gap: 7 }}>
            ...
            <button onClick={doCheckout} disabled={!canCheckOut}
              ...
              {isShared ? `Check Out → ${coReason}` : `Check Out${coDestTeacher ? ` — Notify ${coDestTeacher.full_name.split(' ').pop()}` : ''}`}
            </button>
          </div>
        </div>
      )
    ...
    <>
      ...
      {open && (
        <div style={{
          ...
          <div style={{ padding: '6px 14px 10px', overflowY: 'auto', flex: 1, minHeight: 0 }}>
            <p style={{ fontSize: 9, fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', color: '#ccc', marginBottom: 4 }}>
