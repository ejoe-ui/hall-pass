...
// Used by kiosk and teacher dashboard
...
// Count school days in the current Mon-Fri week
...
export async function fetchTodayScheduleType(date = new Date()) {
  ...
  try {
    ...
    // Calendar fetch failed — fall through to defaults
  }

  // Check calendar event keywords (case-insensitive)
  ...
  // Short week override — use regular all week if ≤4 school days
...
/**
 * Returns current period info based on schedule and current time.
 * @returns {object} { current, next, status }
 *   status: 'period' | 'break' | 'before' | 'after' | 'noSchool'
 *   current: period object or null
 *   next: next period object or null
...
/**
 * Returns checkout status based on 15-minute rule.
 * @returns 'ok' | 'first15' | 'last15'
 */
export function getCheckoutStatus(periodInfo, BLOCK_MINUTES = 15) {
  ...
  const now = new Date()
  const mins = nowMinutes(now)
  const periodStart = toMinutes(current.start)
  const minutesSinceStart = mins - periodStart

  if (minutesSinceStart < BLOCK_MINUTES) return 'first15'
  if (minutesLeftInCurrent <= BLOCK_MINUTES) return 'last15'
  return 'ok'
}
